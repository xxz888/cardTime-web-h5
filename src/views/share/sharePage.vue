    ...
    getBrand() {
      brandQuery(this.global.brandId).then(res => {
          if (res.resp_code == '000000') {
            this.regUrl = res.result.shareMainAddress + '?phone=' + this.phone + '&brand_id=' + this.global.brandId + '&ip=' + this.global.ip
            this.getCanvas()
          } else {
            this.$toast({
      ...
      if (this.showImg == true) {
        ...
        this.$nextTick(() => {
          ...
          setTimeout(() => {
            ...
            cvs.fillText('推荐人:' + this.phone.substr(0, 3) + "****" + this.phone.substr(7), textX, nameT);
