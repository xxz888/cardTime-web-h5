...
<template>
  <div class="online">
    <div class="header theme_bg">
      <van-nav-bar class="online_nav" title="信用卡收款" left-arrow fixed
                   :border='false' @click-left="onClickLeft" right-text="使用说明" @click-right="link('使用说明')"/>
      <div class="online_main">
        ...
        <div class="online_money_box">
          <div class="online_main_money">
            <span>￥</span>
            <van-field class="money_input" :value="money"
                       placeholder="请输入金额，最低100元" readonly/>
          </div>
          <div class="online_main_rate theme">
            <div>
              <div>限额</div>
              <div>2万元/笔</div>
            </div>
        ...
        <div class="cardItem">
          ...
          <div v-if="defDaiCardList.length > 0">
            <img v-if="defDaiCardList[0].logimg"
                 :src="'https://cader-install.oss-cn-shanghai.aliyuncs.com/backManage/bankIcon/BANK_'+defDaiCardList[0].logimg+'.png'"
                 alt="">
            <img v-else src="https://cader-install.oss-cn-shanghai.aliyuncs.com/backManage/bankIcon/BANK_default.png"
                 alt="">
            {{ defDaiCardList[0].bankName }}&nbsp; <span>({{ defDaiCardList[0].cardNo |cardNoEnd }})</span>
          </div>
          <div v-else>
            <img src="https://cader-install.oss-cn-shanghai.aliyuncs.com/backManage/bankIcon/BANK_default.png" alt="">暂无信用卡
          </div>
        ...
        <div class="cardItem">
          ...
          <div v-if="defJieCardList.length > 0">
            <img v-if="defJieCardList[0].logimg"
                 :src="'https://cader-install.oss-cn-shanghai.aliyuncs.com/backManage/bankIcon/BANK_'+defJieCardList[0].logimg+'.png'"
                 alt="">
            <img v-else src="https://cader-install.oss-cn-shanghai.aliyuncs.com/backManage/bankIcon/BANK_default.png"
                 alt="">
          ...
          <div v-else>
            <img src="https://cader-install.oss-cn-shanghai.aliyuncs.com/backManage/bankIcon/BANK_default.png" alt="">暂无储蓄卡
          </div>
    ...
    <!-- 数字键盘 -->
    <van-number-keyboard
      class="online_keyboard"
      v-model="money"
      :show="true"
      :extra-key="['00', '.']"
      close-button-text="立即收款"
      theme="custom"
      @close="setChannel(2)"
      :safe-area-inset-bottom="true">
    </van-number-keyboard>
    <guide :step='step' @next="step == 2||step==10 ? tian(): stepType=false" v-show="stepType"/>

    <!-- 选择更换默认卡 -->
    <van-popup v-model="showPop" position="bottom" closeable
               close-icon-position="top-left" :style="{ height: '50%' }">
    ...
    <cardconfirm ref="cardconfirm" but='1' @disconfirm='tian()'
                 src='https://cader-install.oss-cn-shanghai.aliyuncs.com/backManage/alert.png' :zi='zi'/>
  </div>
</template>
...
import {userBankAndNature, bankcardIsdefault, getchannelBybankcard, newFacadeTopup} from '@/api/online'
...
export default {
  ...
  methods: {
    //-----------获得用户信息-----------
    _getUserMessage() {
      if (localStorage.getItem('realnameStatus') == 1) {
    ...
    getCard(type, nature, isDefault) {
      userBankAndNature(this.userId, type, nature, isDefault).then(res => {
        if (res.resp_code == '000000') {
          ...
          this.zi = type == 2 && nature == 2 ? '您还未添加储蓄卡，是否前往添加？' : '您还未添加信用卡，是否前往添加？'
        } else {
    ...
    tian(type) {
      if (localStorage.getItem('realnameStatus') != 1 && localStorage.getItem('realnameStatus') != null) {
        ...
        if (this.tiantype == 2) {
          ...
          this.$router.push({name: 'debitCardAdd', params: {'type': JSON.stringify(this.tiantype)}})
        }
      }
    },
    seleDefCard(active) {
    ...
    // 设置默认卡
    setDefCard(item) {
      bankcardIsdefault(this.userId, item.cardNo, this.cardType)
        .then(res => {
          if (res.resp_code == '000000') {
            this.showPop = false
            this.getCard(0, 0, 1)
    ...
    setChannel(recommend) {
      ...
      if (this.money == '') {
        this.$toast({message: '单笔交易金额不能为0', position: 'bottom'})
        return
      }
      if (this.money < 100) {
        this.$toast({message: '刷卡金额不能小于100', position: 'bottom'})
        return
      }
      ...
      getchannelBybankcard(this.userId, this.defDaiCardList[0].cardNo, this.money, this.brandId, recommend)
        .then(res => {
          if (res.resp_code == '000000') {
    ...
    // 更换通道
    selChannel() {
      sessionStorage.setItem('selChannel', JSON.stringify(this.channel))
      this.$router.push({
        name: 'selectPassageway',
        params: {item: JSON.stringify(this.defDaiCardList[0]), money: JSON.stringify(this.money)}
      })
    },
    // 确认收款
    toOnline(channel) {
