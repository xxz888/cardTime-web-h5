<template>
  <div>
        ...
        <div class="submit_plan_dec_box">
          ...
          <van-row type="flex">
            ...
            <van-col class="submit_plan_dec" span="12">
              ...
              <span class="submit_plan_dec_cont">{{ task.usedCharge |toFixed }}元</span>
            </van-col>
          </van-row>
        </div>
    ...
    <confirmorcancel ref="causeMessage" src='https://cader-install.oss-cn-shanghai.aliyuncs.com/backManage/alert.png'
                     but='3' :zi='start'/>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getmerchantNameList() {
      ...
      queryMerchant(this.task.version.split("-")[0], this.extra, this.task.bankName, this.item)
        .then(res => {
    ...
    isuserable() { //验证用户是否需要绑卡
      ...
      buildChannel(this.userId, this.task.creditCardNumber).then(res => {
    ...
    confirmPlan() {//执行计划
      ...
      balanceSaveEmptyCard(this.userId, this.task.creditCardNumber, extra).then(res => {
        this.$store.commit('closeLoading')
        if (res.resp_code == "000000") {
          this.$router.push({
            path: "/creditcard",
    ...
    endplanMod() {//终止计划
      this.$store.commit('Loading')
      stopOrder(this.userId, this.task.id).then(res => {
        this.$store.commit('closeLoading')
        if (res.resp_code == '000000') {
          this.$router.push({
            path: "/creditcard",
...
.task_detail_title {
  background: url('https://cader-install.oss-cn-shanghai.aliyuncs.com/backManage/task_detail_title.png') center no-repeat;
...
.task_detail_close {
  background: url('https://cader-install.oss-cn-shanghai.aliyuncs.com/backManage/delete_icon.png') center no-repeat;
  background-size: 100% 100%;
  height: 30px;
  width: 30px;
  margin: 0 auto;
