      ...
      <div class="card_box">
        <div class="card_item">
          ...
          <van-field v-model="item.cardNo" type="digit" label="卡号" placeholder="请输入您的信用卡号" clearable minlength="10"
            ...
            <van-icon name="https://cader-install.oss-cn-shanghai.aliyuncs.com/backManage/addcard/camera.png"
...
export default {
  ...
  methods: {
    ...
    add() {
      addBank(this.token, this.userList.realname, this.item.cardNo, this.userList.idcard, this.item.phone.replace(/\s/g, ""), 0, this.item.expiredTime, this.item.securityCode, this.item.billDay, this.item.repaymentDay, '20000').then(res => {
        if (res.resp_code == "000000") {
