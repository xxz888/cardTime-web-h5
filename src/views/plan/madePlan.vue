  ...
  <div>
            ...
            <van-row type="flex" class="card_item_top" justify="space-between">
              <van-col span="3">
                <div class="card_icon_box">
                  <img class="card_icon"
                    :src="'https://cader-install.oss-cn-shanghai.aliyuncs.com/backManage/bankIcon/BANK_'+item.logimg+'.png'"/>
                </div>
              </van-col>
        ...
        <div class="make_plan_box">
          <div class="make_plan_item van-hairline--bottom">
            <van-row type="flex" justify="space-between">
              ...
              <van-col span="12" class="">
                <van-field class="make_plan_input" v-model="money" placeholder="还款总金额" clearable
                  maxlength="9" minlength="0" type="number" right-icon="edit" input-align="right" @input="moneyInput()"/>
              </van-col>
            </van-row>
          </div>
          <div class="make_plan_item">
            <van-row type="flex" justify="space-between">
              ...
              <van-col span="12" class="">
                <van-field @input="selChannel()" class="make_plan_input" v-model="autoreservedAmount" placeholder="信用卡可用余额"
                  clearable type="number" right-icon="edit" input-align="right"/>
              </van-col>
            </van-row>
          ...
          <div class="make_plan_model van-hairline--bottom">
            ...
            <div class="make_plan_item " v-if="active==2">
              <div class="date_box" v-if="arr.length">
                <div class="date_item" v-for="(item,index) in arr" :key="item.index">
                  <div v-if='index<activeDateLength'>{{ item.split('-')[1] }}/{{ item.split('-')[2] }}</div>
                </div>
              </div>
          ...
          <van-cell class="van-ellipsis" :value="valExtra" is-link @click="diycity()">
            <!-- 使用 title 插槽来自定义标题 -->
            <template #title>
    ...
    <van-popup v-model="planDateTrueFalseBy" round position="bottom" :style="{ height: '63%' }">
      <div class="plan_date_title">
        ...
        <div class="cont">
          <div>制定还款日期</div>
          <!-- <span>(至少选择：{{Math.ceil((money/autoreservedAmount)/dayRepaymentCount)+1}}天)</span> -->
          <span>(至少选择：{{ budget }}天)</span>
        </div>
        <div class="right" @click="dateConfirm()">
          确定
        </div>
      </div>
      <div class="calendar">
        ...
        <div class='content'>
          ...
          <div
            ...
            :key="index"
            :class="item.day >=newDay ? '' : 'gray'"
            @click="item.day >=newDay ? intervalDateSel(item.day,$event):aaa()">
            <span v-if="billDay==item.day.slice(-2)" class="bill color_fff"
                  :class="arr.indexOf(item.day) > -1 ? 'activeDay color_fff' : ''">账单日</span>
            <span v-else-if="repaymentDay==item.day.slice(-2)" class="repment color_fff"
...
export default {
  ...
  activated() {
    ...
    if (!this.$route.meta.isBack) {
      ...
      this.active = '1'
      this.valExtra = '请选择消费地区'
      this.$refs.verificationMethod.twoshow = false
      this.$refs.verificationMethod.Province = '请选择'
      this.$refs.verificationMethod.threeshow = false
      this.$refs.verificationMethod.showCity = false
      this.$refs.verificationMethod.showProvince = true
      this.getAllArr()
    }
  ...
  beforeRouteEnter: (to, from, next) => {
    ...
    if (from.name == 'executeTaskNew' || from.name == 'confirmPlan' || from.name == 'submitPlan') {
      to.meta.isBack = true
    }
    next();
  },
  ...
  methods: {
    getMinBalance() {
      getMinBalanceApi().then(res => {
        if (res.resp_code == "000000") {
          this.minBalance = res.result
        }
      })
    },
    formatMonth(count) {
    ...
    authOnShow(val) {
      if (val == 2 && this.autoreservedAmount < this.minBalance) {
        this.active = '1'
        this.$toast({message: '卡余额需要大于' + this.minBalance + '元才能制定计划，请确认卡余额！', position: 'bottom'})
      } else {
        if (val == 2) {
    ...
    async getPlanDate(item) {
      ...
      let res = await thinkTime()
      if (res.resp_message == '0') {
      ...
      if (+('' + this.year + this.month + billDay) > +('' + this.year + this.month + repaymentDay)) {
        endDay = this.year + '-' + '0' + (Number(this.month) + 1) + '-' + repaymentDay
      } else if (this.month > 12) {
        endDay = (this.year + 1) + '-' + '01' + '-' + repaymentDay
      }
      this.newDay = this.year + "-" + this.month + "-" + this.day;
    ...
    selChannel(type) {
      ...
      if (this.autoreservedAmount < this.minBalance) {
        return;
      }
      ...
      creditcardChannel(
        ...
        this.publicJs.output(res, "查询还款通道");
        if (res.resp_code == '000000') {
          if (res.result.length > 0) {
            this.version = res.result[0].version
            this.task.version = res.result[0].version
            this.$refs.verificationMethod.version = 140
            this.$refs.verificationMethod.channelTag = res.result[0].channelTag
          }
    ...
    createTask() {
      ...
      if (this.autoreservedAmount < this.minBalance) {
        this.$toast({message: '卡余额需要大于' + this.minBalance + '元才能制定计划，请确认卡余额！', position: 'bottom'})
        this.autoreservedAmount = this.minBalance
        return;
      }
      ...
      // 公测用户执行新的计划
      creditcardTaskNew(this.item.userId, this.item.brandId, this.item.cardNo, this.money, reservedAmount, String(this.dateList), this.version, isPoint, this.dayRepaymentCount).then(res => {
        this.$store.commit('closeLoading')
        if (res.resp_code == '000000') {
          if (Array.isArray(res.result)) {
            this.$router.push({
              name: 'submitPlan',
              params: {item: JSON.stringify(this.item), task: JSON.stringify(res), extra: JSON.stringify(this.extra)}
            });
          } else {
            this.$router.push({
              name: 'executeTaskNew',
    ...
    isuserable(item, type, event) {
      this.$router.push({name: type, params: {item: JSON.stringify(item), type: 1}})
    },
    // 日历
    // 获取某年某月总共多少天
...
.van-cell--clickable >>> .van-cell__value .van-cell__title {
  flex: 3;
}
</style>
