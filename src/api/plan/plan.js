...
/** 查询通道是否绑定 */
export function buildChannel(userId,creditCardNumber){
  return request({
...
/**  保存任务接口 (token,POST)  */
export function balanceSaveEmptyCard(userId,creditCardNumber,city){
  return request({
...
/**  生成任务接口 (token,POST)  */
export function creditcardTaskNew(userId, brandId,creditCardNumber,amount, reservedAmount,executeDate,version,isNotPoint,dayRepaymentCount) {
  return request({
